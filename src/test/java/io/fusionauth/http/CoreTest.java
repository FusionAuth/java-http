 ...
 */
@SuppressWarnings({"UastIncorrectHttpHeaderInspection", "OptionalGetWithoutIsPresent"})
public class CoreTest extends BaseTest {
  ...
  public void badPreambleButReset() throws Exception {
    ...
    try (var ignore = makeServer("http", handler, instrumenter).start(); var client = HttpClient.newHttpClient()) {
      sendBadRequest("""
          GET / HTTP/1.1\r
  ...
  public void certificateChain() throws Exception {
    ...
    try (var ignore = makeServer("https", handler).start(); var client = makeClient("https", null)) {
  ...
  public void performance(String scheme) throws Exception {
    ...
    try (var ignore = makeServer(scheme, handler, instrumenter).start(); var client = makeClient(scheme, null)) {
      URI uri = makeURI(scheme, "");
  ...
  public void simpleGetMultiplePorts() throws Exception {
    ...
    try (var ignore = new HTTPServer().withHandler(handler)
